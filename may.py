...
def draw_health_bar():
    pygame.draw.rect(screen, (0, 255, 0), (10, 10, health * 20, 20))  # Adjusted the width
...
def right():
    global spaceship_x
    spaceship_x += 10
    # Check if the spaceship exceeds the right boundary
...
def left():
    global spaceship_x
    spaceship_x -= 10
    # Check if the spaceship exceeds the left boundary
    if spaceship_x < 0:
        spaceship_x = 0
...
power_up_spawn_probability = 0.01  # Adjust as needed
collision_spawn_probability = 0.05  # Adjust as needed
speed_increase_interval = 15  # in seconds
initial_falling_speed = 5
speed_increase_amount = 4

def game_loop():
    global health, shield_end_time, objects, spaceship_x, spaceship_y, falling_speed,background_y
